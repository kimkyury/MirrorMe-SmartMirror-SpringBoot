...
public class OAuthController {
    ...
    @Operation(summary= "Refresh/Access Token 요청", description = "Mirror에서 특정 유저를 조회하기 위한 토큰입니다. ")
    public ApiUtils.ApiResult<ResponseTokensDto> getToken2(
            @RequestParam(name = "userEmail", required = false) String userEmail)  {
    ...
    public ApiUtils.ApiResult<ResponseTokensDto> signUp(
        ...
        ResponseTokensDto tokenDto = oAuthService.confirmLogin(requestLoginDto);
        if ( tokenDto == null){
            return fail(null);
        }
